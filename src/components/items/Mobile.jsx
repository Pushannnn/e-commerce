...
import { mobiles } from '../../data/MobileList.jsx';

const Mobile = ({ onAddToCart, handleAddToCart }) => {

  // Reusable card inside same file
  const MobileCard = ({ id, brand, name, price, image }) => (
    <div className="lg:w-1/4 md:w-1/2 p-4 w-full group">
      <div className="border border-gray-200 rounded-xl p-4 shadow-lg transition-all duration-300 group-hover:shadow-2xl group-hover:border-blue-500 group-hover:scale-105 group-hover:bg-blue-50 relative">
        ...
        <Link to={`/product/mobile/${id}`} className="block relative h-48 rounded overflow-hidden">
          <img
            alt={name}
        ...
        {/* Cart Button */}
        <button
          onClick={() => { onAddToCart(); handleAddToCart({ id, brand, name, price, image }); }}
